...
``test_determinism_n_workers_one_vs_four`` already covers this for a
...
2. **``master_seed`` round-trip.** Episodes from
   ``processes=4`` carry the same ``metadata["master_seed"]`` as
...
def test_runner_serial_and_parallel_produce_identical_episode_lists() -> None:
    """``processes=1`` and ``processes=4`` must produce
    ...
    parallel = Runner(n_workers=4, env_factory=_make_fast_env).run(
        policy_factory=_make_random_policy,
        suite=suite,
    )
...
def test_runner_output_is_sorted_by_cell_then_episode() -> None:
    ...
    assert keys == sorted(keys), (
        f"Runner output not sorted by (cell_index, episode_index) under processes=4 — got {keys}"
    )
